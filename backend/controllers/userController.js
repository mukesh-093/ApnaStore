...
export const login = async(req, res) =>{
    try{
        ...
        const isPasswordValid = await bcrypt.compare(password, exisistingUser.password)
        if(!isPasswordValid){
            return res.status(400).json({
                success:false,
        ...
        const accessToken = jwt.sign({id:exisistingUser._id}, process.env.SECRET_KEY, {expiresIn:'10d'})
        const refreshToken = jwt.sign({id:exisistingUser._id}, process.env.SECRET_KEY, {expiresIn:'30d'})
        exisistingUser.isLoggedIn = true;
        await exisistingUser.save()

        // check for exisiting session and delete it
        const exisitingSession = await Session.findOne({userId:exisistingUser._id})
        if (exisitingSession) {
